...
type ValidatorTests() =
    ...
    member this.``runRules - custom rule - result is ok - returns ok``() = 
        testRunRules 
            <| new Map<string, obj> [||]
            <| [| Custom (fun _ -> ValidationResult.Ok) |]
            <| [| ValidationResult.Ok |]
    ...
    member this.``runRules - custom rule - result is warning - returns warning``() = 
        testRunRules 
            <| new Map<string, obj> [||]
            <| [| Custom (fun _ -> ValidationResult.Warning "Warning") |]
    ...
    member this.``runRules - custom rule - result is error - returns error``() = 
        testRunRules 
            <| new Map<string, obj> [||]
            <| [| Custom (fun _ -> ValidationResult.Error "Error") |]
    ...
    member this.``runRules - variable error rule - variable doesn't exist - returns ok``() = 
        testRunRules 
            <| new Map<string, obj> [||]
            <| [| Variable <| createVariableRule "Var" true ReturnError |]
            <| [| ValidationResult.Ok |]
    ...
    member this.``runRules - variable error rule - is invalid - returns error``() = 
        testRunRules 
            <| new Map<string, obj> [| ("Var", 0 :> obj) |]
            <| [| Variable <| createVariableRule "Var" false ReturnError |]
    ...
    member this.``runRules - variable warning rule - variable doesn't exist - returns ok``() = 
        testRunRules 
            <| new Map<string, obj> [||]
            <| [| Variable <| createVariableRule "Var" true ReturnWarning |]
            <| [| ValidationResult.Ok |]
    ...
    member this.``runRules - variable warning rule - is invalid - returns warning``() = 
        testRunRules 
            <| new Map<string, obj> [| ("Var", 0 :> obj) |]
            <| [| Variable <| createVariableRule "Var" false ReturnWarning |]
